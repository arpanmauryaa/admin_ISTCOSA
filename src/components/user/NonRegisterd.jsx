...
function NonRegisterd({ redioValue }) {
    const [nonRegisterData, setNonRegisterData] = useState()

    const nonRegisterFunction = () => {
        registereduser(redioValue).then(response => {
            const data = response
            setNonRegisterData(data)
        })
    }
    useEffect(() => {
        nonRegisterFunction();
                                        ...
                                        <tr className=" bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            <td scope="row" className="px-2 py-4  text-gray-900  dark:text-white">
                                                <div className='flex'>
                                                    ...
                                                    <UserModal iconValue={item.IsAlive} getFunction={nonRegisterFunction} id={item.RollNumberID} />
